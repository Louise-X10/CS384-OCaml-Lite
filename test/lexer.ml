...
let parse_tests = "test suite for parser" >::: [
    ...
    "tuple" >::
    (fun _ -> assert_equal
        ([LetB("res", [], None, Tuple([CInt 1;CInt 2;CInt 3]))])
        (parse "let res = (1, 2, 3);;"));
    ...
    (fun _ -> assert_equal
        ...
        (parse
            "let res = let f (x : int) : bool = if x < 0 then true else false in f 1;;"));
    ...
    (fun _ -> assert_equal
        ...
        (parse  "let res = match p with | Pair (x, y) => x + y;;"));
